...
import { AlertTriangle, BarChart3, Clock, ReceiptText, ShoppingBag, Wallet } from "lucide-react"
...
export default function VisaoGeralLojaTab() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [salesSummary, alerts, orders, employeesData] = await Promise.all([
  ...
  async function handleShiftReport() {
    ...
    try {
      setShiftReport(await getShiftReport(Number(shiftCashierId), shiftDate))
    } catch {
